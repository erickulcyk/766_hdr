function [ hdr_rgb_img, tone_map, hdr_img, g, weight ] = HDRComplete(fnames, n, channel,hsl_lut, rgb_lut, use_hsl, use_tonemap, g,weight,man)
    if(man==1)
        if(channel==-1)
			...
			if(use_hsl == 1)
				...
				hdr_img = hdr(hdr_img,hsl_imgs,refind, T, g, weight, channel,1,1.5);
			else
    ...
    if(man==0)
        if(channel==-1)
            ...
            if(use_hsl==1)
				...
				hdr_img = squeeze(mean(imgs,1)); %use average value across images for unused channels
				hdr_img = hdr(hdr_img,imgs, refind, T, g, weight, channel,1,1.5);		
			end            
			disp('Done to hdr_img');    
        end
