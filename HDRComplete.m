...
if(man==1)
    if(channel==-1)
        ...
        for i=1:channels
            ...
            disp('Got reference');
            hdr_img = hdr(hdr_img,imgs, refind, T, g, weight, i,deGhost,1.5);% 3);
        ...
        if(use_hsl == 1)
            ...
            hdr_img = hdr(hdr_img,hsl_imgs,refind, T, g, weight, channel,deGhost,1.5);
        else
            hdr_img = squeeze(mean(imgs,1)); %use average value across images for unused channels
            hdr_img = hdr(hdr_img,imgs,refind,T, g, weight, channel,deGhost,1.5);% 3);
        end
        disp('Done to hdr_img');
        
        if(use_tonemap==1)
            tone_map = uint8(round(hdr_img));
            tone_map(:,:,channel) = SimpleToneMap(hdr_img,channel);
            if(use_hsl==1)
...
if(man==0)
    if(channel==-1)
        ...
        if(use_hsl==1)
            hsl_imgs = FramesToHSL(imgs, hsl_lut);
            hdr_img = squeeze(hsl_imgs(2,:,:,:));%mean(hsl_imgs,1)); %use average value across images for unused channels
            disp('Done Convert To HSL');
            hdr_img = hdr(hdr_img,hsl_imgs, refind, T, g, weight, channel,deGhost,1.5);
        else
            hdr_img = squeeze(mean(imgs,1)); %use average value across images for unused channels
            hdr_img = hdr(hdr_img,imgs, refind, T, g, weight, channel,deGhost,1.5);
        end
        disp('Done to hdr_img');
    end
