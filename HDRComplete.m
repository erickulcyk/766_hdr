    ...
    if(man==1)
        if(channel==-1)
            ...
            for i=1:channels
                ...
                disp('Got reference');
                hdr_img = hdr(hdr_img,imgs, refind, T, g, weight, i,deGhost,5);% 3);
			...
			if(use_hsl == 1)
				...
				hdr_img = hdr(hdr_img,hsl_imgs,refind, T, g, weight, channel,deGhost,5);
			else
	            hdr_img = squeeze(mean(imgs,1)); %use average value across images for unused channels
	            hdr_img = hdr(hdr_img,imgs,refind,T, g, weight, channel,deGhost,5);% 3);
    		end
            disp('Done to hdr_img');
    ...
    if(man==0)
        if(channel==-1)
            ...
            if(use_hsl==1)
				...
				hdr_img = squeeze(mean(imgs,1)); %use average value across images for unused channels
				hdr_img = hdr(hdr_img,imgs, refind, T, g, weight, channel,deGhost,5);		
			end            
			disp('Done to hdr_img');    
        end
